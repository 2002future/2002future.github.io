...
var temp = 1;
while (temp <= 4) {
	$("#future_F1>button:nth-child(" + temp + ")").css("background", bg_color[Math.floor(Math.random() * bg_color.length)]);
	$("#future_F1>button:nth-child(" + (temp + 1) + ")").css("background", bg_color[3]);
	$("#future_F1>button:nth-child(" + (temp + 2) + ")").css("background", bg_color[0]);
	temp += 3;
	console.log(temp);
}
